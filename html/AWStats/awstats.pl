...
"<meta name=\"generator\" content=\"AWStats $VERSION from config file awstats.$SiteConfig.conf (http://awstats.sourceforge.net)\"$endtag\n";
		...
		if ($Expires) {
			if (( $BuildReportFormat ne 'xhtml' && $BuildReportFormat eq 'xml' )
			  || ( $ENV{'HTTP_USER_AGENT'} =~ /MSIE|Googlebot/i ) ) {
...
"<meta http-equiv=\"keywords\" content=\"$SiteDomain, free, advanced, realtime, web, server, logfile, log, analyzer, analysis, statistics, stats, perl, analyse, performance, hits, visits\"$endtag\n";
		...
		if ( $FrameName ne 'index' ) {
			...
			if (( $BuildReportFormat ne 'xhtml' && $BuildReportFormat ne 'xml' )
			  || ( $ENV{'HTTP_USER_AGENT'} =~ /MSIE|Googlebot/i )) {
				print "<meta http-equiv=\"Content-Style-Type\" content=\"text/css\"$endtag\n";
			}
		}
...
sub error {
	...
	if ( !$ErrorMessages && $message =~ /^Format error$/i ) {
		...
		if ( scalar keys %HTMLOutput )   { print "<p>n"; }
...
"${tagbold}\"combined log format\"${tagunbold} like this:${tagbr}\n";
				print( scalar keys %HTMLOutput ? "$tagfontgrey<em>" : "" );
				print
...
"LogFormat parameter is wrong in config/domain file. Value is '$LogFormat' (should be 1,2,3,4,5 or a 'personalized AWStats log format string')"
	...
	$DirJs        ||= '/AWStats/js';
	$DirCss       ||= '/AWStats/css';
	$StyleSheet   ||= '/AWStats/css/awstats.css';
	if ( $DNSLookup !~ /[0-2]/ ) {
		error(
...
sub ShowFormFilter {
	...
	if ( !$StaticLinks ) {
		...
		print "\n<form"
		  ...
		  . "\" class=\"aws_border\">\n";
		print
"<table width=\"99%\" border=\"0\" cellspacing=\"0\" cellpadding=\"2\" summary=\"FormFilter\">\n<col width=\"100\"$endtag<col width=\"100\"$endtag<col$endtag<col width=\"100\"$endtag<col width=\"100\"$endtag\n<tr>\n";
		print "<td align=\"left\">$Message[79]&nbsp;:</td>\n";
		print
"<td align=\"left\"><input type=\"text\" name=\"${fieldfiltername}\" value=\"$fieldfilterinvalue\" class=\"aws_formfield\" tabindex=\"3\" accesskey=\"3\"$endtag</td>\n";
		print "<td></td>";
		print "<td align=\"left\">$Message[153]&nbsp;:</td>\n";
		print
"<td align=\"left\"><input type=\"text\" name=\"${fieldfiltername}ex\" value=\"$fieldfilterexvalue\" class=\"aws_formfield\" tabindex=\"4\" accesskey=\"4\"$endtag</td>\n";
		print "<td>";
		...
		if ( $menulink->{$key} == 2 ) {
			...
			print "<a href=\""
			  . (
				...
				: "$PROG$StaticLinks . uc($key) . $StaticExt"
			  )
...
#	print "$Center<a name=\"EMAILSENDERS\" id=\"EMAILSENDERS\">&nbsp;</a><br$endtag\n";
	my $title;
...
"$Message[131] ($Message[77] $MaxNbOf{'EMailsShown'}) &nbsp; - &nbsp; <a href=\""
	...
	print "\n";
	print
"<tr bgcolor=\"#$color_TableBGRowTitle\"><th colspan=\"3\" abbr=\"$Message[131]\">$Message[131] : "
...
"<th rowspan=\"2\" class=\"colork\" abbr=\"$Message[106]\">$Message[106]</th>";
	...
	if ( $ShowEMailSenders =~ /L/i ) {
		print "<th rowspan=\"2\" width=\"120\" abbr=\"$Message[9]\">$Message[9]</th>";
	}
	print "</tr>\n";
	print
"<tr bgcolor=\"#$color_TableBGRowTitle\"><th abbr=\"Local\">Local</th><td></td><th abbr=\"External\">External</th></tr>";
...
#	print "$Center<a name=\"EMAILRECEIVERS\" id=\"EMAILRECEIVERS\">&nbsp;</a><br$endtag\n";
	my $title;
...
"AWStats history file name must match following syntax: ${PROG}MMYYYY[.config].txt",
	...
	print $ENV{'GATEWAY_INTERFACE'} ? "<br$endtag\n" : "\n";
...
if ( $FrameName eq 'index' ) {
	...
	print "<frame name=\"mainleft\" src=\""
	  ...
	  . "\" noresize=\"noresize\" frameborder=\"0\"$endtag\n";
	print "<frame name=\"mainright\" src=\""
	  . XMLEncode("$AWScript?${NewLinkParams}framename=mainright")
	  . "\" noresize=\"noresize\" scrolling=\"yes\" frameborder=\"0\"$endtag\n";
	print "<noframes><body>";
	print "Your browser does not support frames.<br$endtag\n";
	print "You must set AWStats UseFramesWhenCGI parameter to 0\n";
	print "to see your reports.<br$endtag\n";
	print "</body></noframes>\n";
	print "</frameset>\n";
	&html_end(0);
	exit 0;
...
s/^(cache|related):[^\+]+//
													...
													$param = &DecodeEncodedString($param);
...
# TODO If there was no lines parsed (log was empty), we only update LastUpdate line with YYYYMMDDHHMMSS 0 0 0 0 0
	...
	if ($PurgeLogFile) {
		if ($ArchiveLogRecords) {
			...
			open( LOG, "+<$LogFile" )
			  || error(
"Enable to archive log records of \"$LogFile\" into \"$ArchiveFileName\" because source can't be opened for read and write: $!<br$endtag\n"
			  );
		}
		else {
			open( LOG, "+<$LogFile" );
...
"<table class=\"aws_data sortable\" border=\"0\" cellpadding=\"2\" cellspacing=\"0\" width=\"100%\" summary=\"Menu\">\n";
		...
		if ( $FrameName ne 'mainright' ) {
			...
			if ( $FrameName eq 'mainleft' ) {
				...
				print
"<tr><td class=\"awsm\"><strong>$Message[7]:</strong></td></tr><tr><td class=\"aws\"><span style=\"font-size: 12px;\">$shortSiteDomain</span></td>";
			}
			else {
				print
	...
	if ( $ShowMenu || $FrameName eq 'mainleft' ) {
		...
		{    # If main page asked
			...
			if ( !$PluginsLoaded{'ShowMenu'}{'menuapplet'} ) {
				...
				if ($linetitle) {
					print "<col width=\"$WIDTHMENU1\"$endtag";
				}
...
#		print "$Center<a name=\"DOMAINS\" id=\"DOMAINS\">&nbsp;</a><br$endtag\n";
...
"<tr><td><img src=\"$DirIcons\/flags\/$newkey.png\" height=\"14\" width=\"14\""
    			...
    			print "<td class=\"aws\" style=\"line-height:6px\">";
				if ( $ShowDomainsStats =~ /P/i ) {
					print
...
#		print "$Center<a name=\"HOSTS\" id=\"HOSTS\">&nbsp;</a><br$endtag\n";
			...
			if ( $FilterIn{'host'} || $FilterEx{'host'} ) {    # With filter
				...
				if ( $FilterEx{'host'} ) {
					print " Exlude $Message[79] '<strong>$FilterEx{'host'}</strong>'";
				}
				if ( $FilterIn{'host'} || $FilterEx{'host'} ) { print ": "; }
				...
				if ( $MonthRequired ne 'all' ) {
					if ( $HTMLOutput{'allhosts'} || $HTMLOutput{'lasthosts'} ) {
						print
"<br$endtag$Message[102]: $TotalHostsKnown $Message[82], $TotalHostsUnknown $Message[1] - $TotalUnique $Message[11]";
					}
    			}
    		}
    		else {	# Without filter
...
"<th class=\"colork\" abbr=\"$Message[75]\">$Message[75]</th>";
    		...
    		foreach my $key (@keylist) {
				...
				print "<tr><td class=\"aws\">"
				  . ( $_robot_l{$key} ? '<strong>'  : '' ) . "$host"
				  . ( $_robot_l{$key} ? '</strong>' : '' ) . "</td>";
    			&ShowHostInfo($key);
				if ( $ShowHostsStats =~ /P/i ) {
					print "<td>"
...
#		print "$Center<a name=\"UNKNOWNIP\" id=\"UNKNOWNIP\">&nbsp;</a><br$endtag\n";
...
#		print "$Center<a name=\"ROBOTS\" id=\"ROBOTS\">&nbsp;</a><br$endtag\n";
			my $title = '';
...
#		print "$Center<a name=\"URLS\" id=\"URLS\">&nbsp;</a><br$endtag\n";
		...
		print "\n";
			...
			if ( $FilterIn{'url'} || $FilterEx{'url'} ) {
				...
				if ( $FilterEx{'url'} ) {
					print "Exclude $Message[79] <strong>$FilterEx{'url'}</strong>";
				}
				if ( $FilterIn{'url'} || $FilterEx{'url'} ) { print ": "; }
				...
				if ( $MonthRequired ne 'all' ) {
					if ( $HTMLOutput{'urldetail'} ) {
						print
"<br$endtag$Message[102]: $TotalDifferentPages $Message[28]";
    			}
    		}
			}
...
#		print "$Center<a name=\"UNKNOWNOS\" id=\"UNKNOWNOS\">&nbsp;</a><br$endtag\n";
			my $title = "$Message[46]";
...
#		print "$Center<a name=\"UNKNOWNBROWSER\" id=\"UNKNOWNBROWSER\">&nbsp;</a><br$endtag\n";
			my $title = "$Message[50]";
...
"<tr><td class=\"aws\"><span style=\"color: #$color_other\">$Message[2]</span></td>";
		...
		if ( $HTMLOutput{'osdetail'} ) {
    		...
    		# Show os versions
#		print "$Center<a name=\"OSVERSIONS\" id=\"OSVERSIONS\">&nbsp;</a><br$endtag";
			my $title = "$Message[59]";
			&tab_head( "$title", 19, 0, 'osversions' );
		print "<col width=\"$WIDTHCOLICON\"$endtag<col width=\"$WIDTHOS\"$endtag<col width=\"$WIDTHHIT\"$endtag<col width=\"$WIDTHPARCENT\"$endtag<col$endtag\n";
			print
				...
				foreach my $key (sort{ $_os_h{ $b } <=> $_os_h{ $a }} keys %_os_h) { # ヒットの多い順にソート
					if ( $key =~ /^$family(.*)/i ) {
						if ( !$familyheadershown ) {
							...
							print
"<tr style=\"background-color:#F6F6F6\"><td class=\"aws\" colspan=\"2\"><strong>$family_name</strong></td>";
							print "<td><strong>"
						...
						if ($ShowOSStats) {
							print
"<img src=\"$DirIcons\/other\/$BarPng{'hh'}\" width=\"$bredde_h\" height=\"5\"".AltTitle("$bredde_h")."$endtag";
						}
    					print "</td>";
    					print "</tr>\n";
    					$count++;
    		...
    		foreach my $key (@keylist) {
				...
				if ( !$familyheadershown )  {
					...
					print
"<tr style=\"background-color:#F6F6F6\"><td class=\"aws\" colspan=\"2\"><strong>$Message[2]</strong></td>";
					print "<td><strong>"
					  . ( $total_h - $TotalFamily )
					  . "</strong></td><td><strong>$p</strong></td><td></td>";
					print "</tr>\n";
...
#		print "$Center<a name=\"BROWSERSVERSIONS\" id=\"BROWSERSVERSIONS\">&nbsp;</a><br$endtag";
			my $title = "$Message[21]";
			  ...
			  )
			{
    			...
    			foreach my $key ( reverse sort SortBrowsers keys %_browser_h ) {
					if ( $key =~ /^$family(.*)/i ) {
						if ( !$familyheadershown ) {
							print
"<tr style=\"background-color:#F6F6F6\"><td class=\"aws\" colspan=\"2\"><strong>"
							  . uc($family)
							  . "</strong></td>";
...
#		print "$Center<a name=\"REFERERSE\" id=\"REFERERSE\">&nbsp;</a><br$endtag\n";
			my $title = "$Message[40]";
...
"<col$endtag<col width=\"$WIDTHPAGE\"$endtag<col width=\"$WIDTHPARCENT\"$endtag<col width=\"$WIDTHHIT\"$endtag<col width=\"$WIDTHPARCENT\"$endtag\n";
			print
"<tr class=\"colortab\"><th abbr=\"$TotalDifferentSearchEngines\">$TotalDifferentSearchEngines $Message[122]</th>";
			print
...
#		print "$Center<a name=\"REFERERPAGES\" id=\"REFERERPAGES\">&nbsp;</a><br$endtag\n";
		...
		print "<col$endtag<col width=\"$WIDTHPAGE\"$endtag<col width=\"$WIDTHPARCENT\"$endtag<col width=\"$WIDTHHIT\"$endtag<col width=\"$WIDTHPARCENT\"$endtag\n";
    		print "<tr class=\"colortab\"><th abbr=\"$Message[28]\">";
			if ( $FilterIn{'refererpages'} || $FilterEx{'refererpages'} ) {

				if ( $FilterIn{'refererpages'} ) {
					print "$Message[79] <strong>$FilterIn{'refererpages'}</strong>";
				}
				if ( $FilterIn{'refererpages'} && $FilterEx{'refererpages'} ) {
					print " - ";
				}
				if ( $FilterEx{'refererpages'} ) {
					print
					  "Exclude $Message[79] <strong>$FilterEx{'refererpages'}</strong>";
				}
				if ( $FilterIn{'refererpages'} || $FilterEx{'refererpages'} ) {
					print ": ";
				}
...
#if ($MonthRequired ne 'all') {
#	if ($HTMLOutput{'refererpages'}) { print "<br$endtag$Message[102]: $TotalDifferentPages $Message[28]"; }
#}
    		}
    		else { print "$Message[102]: $cpt $Message[28]"; }
    		print "</th>";
...
#		print "$Center<a name=\"KEYPHRASES\" id=\"KEYPHRASES\">&nbsp;</a><br$endtag\n";
			...
			print "<tr class=\"colortab\""
			  . Tooltip(15)
			  . "><th abbr=\"$TotalDifferentKeyphrases\">$TotalDifferentKeyphrases $Message[103]</th><th class=\"colors\" abbr=\"$Message[14]\">$Message[14]</th><th class=\"colors\" abbr=\"$Message[15]\">$Message[15]</th></tr>\n";
			$total_s = 0;
			my $count = 0;
			&BuildKeyList(
...
"<tr><td class=\"aws\"><span style=\"color: #$color_other\">$Message[124]</span></td><td>$rest_s</td>";
		...
		if ( $HTMLOutput{'keywords'} ) {
#		print "$Center<a name=\"KEYWORDS\" id=\"KEYWORDS\">&nbsp;</a><br$endtag\n";
			...
			print "<tr class=\"colortab\""
			  . Tooltip(15)
			  . "><th abbr=\"$TotalDifferentKeywords\">$TotalDifferentKeywords $Message[13]</th><th class=\"colors\" abbr=\"$Message[14]\">$Message[14]</th><th class=\"colors\" abbr=\"$Message[15]\">$Message[15]</th></tr>\n";
			$total_s = 0;
			my $count = 0;
			foreach my $key (@keylist) {
				my $mot;
...
#			print "$Center<a name=\"ERRORS$code\" id=\"ERRORS$code\">&nbsp;</a><br$endtag\n";
...
"<td class=\"aws\">$ExtraFirstColumnFormat[$extranum]</td>",
				...
				if ( $ExtraAddSumRow[$extranum] ) {
		 			print "<tr>";
		 			print "<td class=\"aws\"><strong>$Message[102]</strong></td>";
					if ( $ExtraStatTypes[$extranum] =~ m/P/i ) {
						print "<td>" . ($total_p) . "</td>";
					}
					if ( $ExtraStatTypes[$extranum] =~ m/H/i ) {
		...
		if ( $HTMLOutput{'info'} ) {
    		...
    		# Not yet available
#		print "$Center<a name=\"INFO\" id=\"INFO\">&nbsp;</a><br$endtag";
    		&html_end(1);
    	}
		...
		if ( $htmloutput =~ /^plugin_(\w+)$/ ) {
			my $pluginname = $1;
    		print "$Center<a name=\"plugin_$pluginname\">&nbsp;</a><br$endtag";
	...
	if ( $HTMLOutput{'main'} ) {
		...
		if ($ShowSummary) {
			...
			#print "$Center<a name=\"summary\">&nbsp;</a><br$endtag\n";
...
"<td></td>";
			...
			if ( $LogType eq 'M' ) {
				...
				print "<td></td>\n";
				if ( $ShowSummary =~ /H/i ) {
					print "<td><strong>$TotalHits</strong>"
					  . (
						$LogType eq 'M'
						  ? ""
						  : "<br$endtag($RatioHits&nbsp;"
				...
				if ( $ShowSummary =~ /B/i ) {
					print "<td><strong>"
					  ...
					  . "</strong><br$endtag($RatioBytes&nbsp;$Message[108]/"
				...
				if ( $ShowSummary =~ /U/i ) {
					print "<td>"
					  . (
						$MonthRequired eq 'all'
						  ? "<strong>&lt;= $TotalUnique</strong><br$endtag$Message[129]"
				...
				if ( $ShowSummary =~ /V/i ) {
					print
"<td><strong>$TotalVisits</strong><br$endtag($RatioVisits&nbsp;$Message[52])</td>";
				}
				else { print "<td></td>"; }
				if ( $ShowSummary =~ /P/i ) {
					print "<td><strong>$TotalPages</strong><br$endtag($RatioPages&nbsp;"
					  . $Message[56] . "/"
					  . $Message[12]
					  . ")</td>";
				}
			...
			if ( $LogType eq 'M' || $LogType eq 'W' || $LogType eq 'S' ) {
				...
				if ( $LogType eq 'M' ) {
    				...
    				print "<td></td>\n";
					if ( $ShowSummary =~ /H/i ) {
						print "<td><strong>$TotalNotViewedHits</strong></td>";
					}
					else { print "<td></td>"; }
					if ( $ShowSummary =~ /B/i ) {
						print "<td><strong>"
					...
					if ( $ShowSummary =~ /P/i ) {
						print "<td><strong>$TotalNotViewedPages</strong></td>";
					}
					else { print "<td></td>"; }
					if ( $ShowSummary =~ /H/i ) {
						print "<td><strong>$TotalNotViewedHits</strong></td>";
...
#			print "$Center<a name=\"MONTH\" id=\"MONTH\">&nbsp;</a><br$endtag\n";
...
"<tr><td class=\"colortab\">$Message[102]</td>";
				if ( $ShowMonthStats =~ /U/i ) {
					print
					  "<td class=\"colortab\">$total_u</td>";
				}
				if ( $ShowMonthStats =~ /V/i ) {
					print
					  "<td class=\"colortab\">$total_v</td>";
				}
				if ( $ShowMonthStats =~ /P/i ) {
					print
					  "<td class=\"colortab\">$total_p</td>";
				}
				if ( $ShowMonthStats =~ /H/i ) {
					print
					  "<td class=\"colortab\">$total_h</td>";
				}
				if ( $ShowMonthStats =~ /B/i ) {
...
#			print "$Center<a name=\"DAYSOFWEEK\" id=\"DAYSOFWEEK\">&nbsp;</a><br$endtag\n";
			my $title = "$Message[91]";
...
#			print "$Center<a name=\"HOURS\" id=\"HOURS\">&nbsp;</a><br$endtag\n";
			my $title = "$Message[20]";
...
#			print "$Center<a name=\"COUNTRIES\" id=\"COUNTRIES\">&nbsp;</a><br$endtag\n";
			my $title =
...
"<tr class=\"colortab\"><th colspan=\"3\" abbr=\"$Message[17]\">$Message[17]</th>";
			...
			if ( $ShowDomainsStats =~ /U/i ) {
				print "<th class=\"coloru\" abbr=\"$Message[11]\""
				  . Tooltip(2)
				  . ">$Message[11]</th>";
			}
			if ( $ShowDomainsStats =~ /V/i ) {
				print "<th class=\"colorv\" abbr=\"$Message[10]\""
				  . Tooltip(1)
				  . ">$Message[10]</th>";
			}
			if ( $ShowDomainsStats =~ /P/i ) {
				print "<th class=\"colorp\" abbr=\"$Message[56]\""
				  . Tooltip(3)
				  . ">$Message[56]</th>";
			}
			if ( $ShowDomainsStats =~ /H/i ) {
				print "<th class=\"colorh\" abbr=\"$Message[57]\""
				  . Tooltip(4)
				  . ">$Message[57]</th>";
			}
			if ( $ShowDomainsStats =~ /B/i ) {
				print "<th class=\"colork\" abbr=\"$Message[75]\""
				  . Tooltip(5)
				  . ">$Message[75]</th>";
			}
			print "<td></td>";
...
#			print "$Center<a name=\"VISITORS\" id=\"VISITORS\">&nbsp;</a><br$endtag\n";
			my $title =
"$Message[81] ($Message[77] $MaxNbOf{'HostsShown'}) &nbsp; - &nbsp; <a href=\""
			...
			if ( $MonthRequired ne 'all' ) {
				print
"$Message[81] : $TotalHostsKnown $Message[82], $TotalHostsUnknown $Message[1]<br$endtag$TotalUnique $Message[11]</th>";
			}
			else {
			...
			if ( $ShowHostsStats =~ /B/i ) {
				print "<th class=\"datasize colork\" abbr=\"$Message[75]\""
				  . Tooltip(5)
				  . ">$Message[75]</th>";
			}
			if ( $ShowHostsStats =~ /L/i ) {
...
#			print "$Center<a name=\"LOGINS\" id=\"LOGINS\">&nbsp;</a><br$endtag\n";
			my $title =
...
#			print "$Center<a name=\"WORMS\" id=\"WORMS\">&nbsp;</a><br$endtag\n";
			...
			print "<th abbr=\"$Message[164]\">" . ( scalar keys %_worm_h ) . " $Message[164]*</th>";
...
#			print "$Center<a name=\"SESSIONS\" id=\"SESSIONS\">&nbsp;</a><br$endtag\n";
			...
			print "<tr class=\"colortab\""
			  . Tooltip(1)
			  . "><th abbr=\"$Message[10]\">$Message[10]: $TotalVisits - $Message[96]: $average_s s</th><th class=\"colors\" abbr=\"$Message[10]\">$Message[10]</th><th class=\"colors\" abbr=\"$Message[15]\">$Message[15]</th></tr>\n";
			$average_s = 0;
			$total_s   = 0;
			my $count = 0;
...
#			print "$Center<a name=\"FILETYPES\" id=\"FILETYPES\">&nbsp;</a><br$endtag\n";
			my $Totalh = 0;
...
"<th class=\"colork\" abbr=\"$Message[100]\">$Message[100]</th><th class=\"colork\" abbr=\"$Message[101]\">$Message[101]</th><th class=\"colork\" abbr=\"$Message[99]\">$Message[99]</th>";
			...
			foreach my $key (@keylist) {
				...
				if ( $key eq 'Unknown' ) {
					...
					print "<tr><td><img src=\"$DirIcons\/mime\/$nameicon.png\""
					  . AltTitle("")
			...
			if ( $ShowFileTypesStats =~ /C/i ) {
				...
				if ( $ShowFileTypesStats =~ /B/i ) { $colspan += 2; }
	 			print "<tr>";
				print
"<td class=\"aws\" colspan=\"$colspan\"><strong>$Message[98]</strong></td>";
				if ( $ShowFileTypesStats =~ /C/i ) {
					if ($total_con) {
						my $percent =
...
#			print "$Center<a name=\"URLS\" id=\"URLS\">&nbsp;</a><a name=\"ENTRY\" id=\"ENTRY\">&nbsp;</a><a name=\"EXIT\" id=\"EXIT\">&nbsp;</a><br$endtag\n";
			my $title =
"$Message[19] ($Message[77] $MaxNbOf{'PageShown'}) &nbsp; - &nbsp; <a href=\""
			...
			print "\n";
			print
"<tr class=\"colortab\"><th abbr=\"$TotalDifferentPages\">$TotalDifferentPages $Message[28]</th>";
...
#			print "$Center<a name=\"OS\" id=\"OS\">&nbsp;</a><br$endtag\n";
			my $Totalh   = 0;
			my %new_os_h = ();
			...
			foreach my $key (@keylist) {
				...
				if ( $key eq 'Unknown' ) {
					print "<tr><td"
					  ...
					  . AltTitle("unknown")
					  . " width=\"14\" height=\"14\"$endtag</td><td class=\"aws\"><span style=\"color: #$color_other\">$Message[0]</span></td><td>$_os_h{$key}</td><td>$p</td></tr>\n";
				}
				else {
					my $keywithoutcumul = $key;
					$keywithoutcumul =~ s/cumul$//i;
					...
					print "<tr><td"
					  ...
					  . AltTitle("$nameicon")
					  . " width=\"14\" height=\"14\"$endtag</td><td class=\"aws\">$libos</td><td>$new_os_h{$key}</td><td>$p</td></tr>\n";
				}
...
#			print "$Center<a name=\"BROWSERS\" id=\"BROWSERS\">&nbsp;</a><br$endtag\n";
			my $Totalh        = 0;
			my %new_browser_h = ();
...
#			print "$Center<a name=\"SCREENSIZES\" id=\"SCREENSIZES\">&nbsp;</a><br$endtag\n";
			my $Totalh = 0;
...
#		print "\n<a name=\"REFERING\" id=\"REFERING\">&nbsp;</a>\n\n";

		# BY REFERENCE
		#---------------------------
		if ($ShowOriginStats) {
			if ($Debug) { debug( "ShowOriginStats", 2 ); }
#			print "$Center<a name=\"REFERER\" id=\"REFERER\">&nbsp;</a><br$endtag\n";
			my $Totalp = 0;
			foreach ( 0 .. 5 ) {
				$Totalp +=
...
"<th class=\"colorh\" abbr=\"$Message[57]\">$Message[57]</th><th class=\"colorh\" abbr=\"$Message[15]\">$Message[15]</th>";
			...
			print "<tr"
			  . Tooltip(13)
			  . "><td class=\"aws\"><strong>$Message[40]</strong> - <a href=\""
			  . (
				$ENV{'GATEWAY_INTERFACE'}
					|| !$StaticLinks
...
#				print "<table summary=\"Referrals by search engines\">\n";
				$total_p = 0;
				$total_h = 0;
				my $count = 0;
				my $p_pt;
...
"Total real / shown : $TotalSearchEnginesPages / $total_p -  $TotalSearchEnginesHits / $total_h",
				...
				if ( $rest_p > 0 || $rest_h > 0 ) {
					print
"<tr><td class=\"aws\"><span style=\"color: #$color_other\">- $Message[2]</span></td>";
					print "<td>$rest_p</td><td>$p_pt %</td>";
					print "<td>$rest_h</td><td>$p_ht %</td>";
					print "</tr>\n";
				}
#				print "</table>";
			}
			...
			print "<tr"
			  . Tooltip(14)
			  . "><td class=\"aws\"><strong>$Message[41]</strong> - <a href=\""
			  . (
				$ENV{'GATEWAY_INTERFACE'}
					|| !$StaticLinks
...
#		if ( $ShowKeyphrasesStats || $ShowKeywordsStats ) { print "<br$endtag\n"; }
		if ( $ShowKeyphrasesStats && $ShowKeywordsStats ) {
			print
			  "<table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\" summary=\"Key Words\" id=\"KEYS\"><col width=\"50%\"$endtag<col$endtag<col width=\"50%\"$endtag\n<tr>";
		}
		if ($ShowKeyphrasesStats) {
			...
			&tab_head(
"$Message[120] ($Message[77] $MaxNbOf{'KeyphrasesShown'})<br$endtag<a href=\""
				  . (
					  $ENV{'GATEWAY_INTERFACE'}
						|| !$StaticLinks
...
"<tr><td class=\"aws\"><span style=\"color: #$color_other\">$Message[124]</span></td><td>$rest_s</td>";
		...
		if ($ShowKeywordsStats) {
			...
			&tab_head(
"$Message[121] ($Message[77] $MaxNbOf{'KeywordsShown'})<br$endtag<a href=\""
				  . (
					  $ENV{'GATEWAY_INTERFACE'}
						|| !$StaticLinks
...
#			print "$Center<a name=\"MISC\" id=\"MISC\">&nbsp;</a><br$endtag\n";
...
#			print "$Center<a name=\"ERRORS\" id=\"ERRORS\">&nbsp;</a><br$endtag\n";
			my $title = "$Message[32]";
...
#			print "$Center<a name=\"CLUSTERS\" id=\"CLUSTERS\">&nbsp;</a><br$endtag\n";
			...
			if ($ShowClusterStats =~ /P/i) { print "<th class=\"colorp\" abbr=\"$Message[56]\">$Message[56]</th><th class=\"colorp\" abbr=\"$Message[15]\">$Message[15]</th>"; }
			if ($ShowClusterStats =~ /H/i) { print "<th class=\"colorh\" abbr=\"$Message[57]\">$Message[57]</th><th class=\"colorh\" abbr=\"$Message[15]\">$Message[15]</th>"; }
			if ($ShowClusterStats =~ /B/i) { print "<th class=\"colork\" abbr=\"$Message[75]\">$Message[75]</th><th class=\"colork\" abbr=\"$Message[15]\">$Message[15]</th>"; }
...
# total for cluster section only, to calculate ratio, instead of using global total
		...
		foreach my $extranum ( 1 .. @ExtraName - 1 ) {
			if ($Debug) { debug( "ExtraName$extranum", 2 ); }
#	 		print "$Center<a name=\"EXTRA$extranum\" id=\"EXTRA$extranum\">&nbsp;</a><br$endtag";
			my $title = $ExtraName[$extranum];
			&tab_head( "$title", 19, 0, "extra$extranum" );
			print "<col$endtag";
	 		if ($ExtraStatTypes[$extranum] =~ m/P/i) { print "<col width=\"$WIDTHPAGE\"$endtag"; }
