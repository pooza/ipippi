...
<td style="width:528px;padding:2px 0px;" class="bg_06"><span class="b_b c_00">プロフィール入力&nbsp;&nbsp;(&nbsp;<span class="caution">※</span>の項目は必須です)</span></td>
</tr>
</table>
</div>
<!-- ここまで：小タイトル -->
...
({elseif $profile.form_type == 'checkbox'})
	...
	({counter name=$profile.name assign=_cnt})
	({if $_cnt % 3 == 1})<tr>({/if})
		<td><input type="checkbox" name="profile[({$profile.name})][]" value="({$item.c_profile_option_id})"({if $profs.profile[$profile.name].value && in_array($item.value, $profs.profile[$profile.name].value)}) checked="checked"({/if})></td>
		<td>({$item.value|default:"--"})</td>
	({if $_cnt % 3 == 0})</tr>({/if})
	({/foreach})
	({if $_cnt % 3 != 0})</tr>({/if})
...
({html_options options=$public_flags selected=$profs.profile[$profile.name].public_flag})
</select>
...
パスワード確認用 <span class="caution">※</span>
