...
({if $is_prev})<a href="page.php?p=h_bookmark_list&direc=-1&page=({$page})">前の20名を表示</a>&nbsp;&nbsp;({/if})
({$pager_index.displaying_first})名〜({$pager_index.displaying_last})名を表示
({if $is_next})&nbsp;&nbsp;<a href="page.php?p=h_bookmark_list&direc=1&page=({$page})">次の20名を表示</a>({/if})
...
追加したユーザーが退会された場合は空欄で表示されます。
自動で削除はされませんので、お手数ですが「削除する」ボタンをクリックしてください。
...
<a href="page.php?p=f_home&target_c_member_id=({$c_member.c_member_id})"><img src="./skin/btn/shousai.gif" alt="詳細を見る" class="icon"></a>&nbsp;
<a href="do.php?cmd=bookmark_delete&target_c_member_id=({$c_member.c_member_id})&amp;sessid=({$PHPSESSID})"><img src="./skin/btn/sakujo.gif" alt="削除する" class="icon"></a>
