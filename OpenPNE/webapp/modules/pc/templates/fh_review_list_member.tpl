...
<td align="left">
&nbsp;<a href="review_help.pl">おすすめレビューを活用しよう</a>
</td>
</tr>
*})
<tr>
...
({if $is_prev})<a href="page.php?p=fh_review_list_member&c_member_id=({$c_member.c_member_id})&direc=-1&page=({$page})">＜前</a> ({/if})
({$start_num})件〜({$end_num})件を表示
({if $is_next})<a href="page.php?p=fh_review_list_member&c_member_id=({$c_member.c_member_id})&direc=1&page=({$page})">次＞</a>({/if})
...
<a href="page.php?p=h_review_list_product&c_review_id=({$review.c_review_id})">全てのレビューを見る(({$review.write_num}))</a>

({if $type eq "h"})|<a href="page.php?p=h_review_add_write&category_id=({$review.c_review_category_id})&asin=({$review.asin})">編集</a><br>({/if})
