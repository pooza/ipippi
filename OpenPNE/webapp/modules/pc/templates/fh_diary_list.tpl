...
<div><a href="page.php?p=fh_diary_list&target_c_member_id=({$target_member.c_member_id})&year=({$date.year})&month=({$date.month})"><img src="./skin/dummy.gif" style="width:14px;height:14px;" class="icon icon_2">({$date.year})年({$date.month})月の一覧</a></div>
...
<tr>
<td style="width:36px;" class="bg_06"><img src="./skin/content_header_1.gif" style="width:30px;height:20px;" class="dummy"></td>
<td style="width:488px;padding:2px 0px;" class="bg_06"><span class="b_b c_00">日記を書く</span></td>
</tr>
</table>
<!-- ここまで：小タイトル -->
({*ここまで：header*})
...
({if $is_prev})
<a href="page.php?p=fh_diary_list&target_c_member_id=({$target_member.c_member_id})&direc=-1&page=({$page})">前を表示</a>
({/if})
...
件を表示
({if $is_next})
<a href="page.php?p=fh_diary_list&target_c_member_id=({$target_member.c_member_id})&direc=1&page=({$page})">次を表示</a>
({/if})
...
<a href="page.php?p=fh_diary&target_c_diary_id=({$item.c_diary_id})">コメント(({$item.num_comment}))</a>

|<a href="./page.php?p=fh_diary&target_c_diary_id=({$item.c_diary_id})">もっと読む</a>

({if $type == "h"})
|<a href="page.php?p=h_diary_edit&target_c_diary_id=({$item.c_diary_id})&target_c_member_id=({$target_member.c_member_id})">編集</a>
({/if})
