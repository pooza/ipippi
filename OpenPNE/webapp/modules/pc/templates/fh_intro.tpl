...
<td style="width:598px;padding:2px 0px;" class="bg_06"><span class="b_b c_00">({$WORD_MY_FRIEND})からの紹介文</span></td>
</tr>
</table>
</div>
<!-- ここまで：小タイトル -->
...
<br><a href="page.php?p=f_intro_edit&target_c_member_id=({$item.c_member_id})">この({$WORD_MY_FRIEND})を紹介する</a>
({/if})
<br><a href="do.php?cmd=f_intro_from_delete&target_c_member_id=({$item.c_member_id})&amp;sessid=({$PHPSESSID})">削除</a>
({/if})
({if $item.c_member_id==$member.c_member_id})
<br><a href="page.php?p=f_intro_edit&target_c_member_id=({$target_member.c_member_id})">編集</a>
({/if})
